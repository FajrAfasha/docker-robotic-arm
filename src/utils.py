...
class RobotArmEnv:
    def __init__(self):
        ...
        self.joint_angle_arrays = [  # Store the discrete angle arrays for each joint
            np.linspace(-np.pi, np.pi, 30),  # Joint 1 angles
            np.linspace(-np.pi / 3, np.pi / 3, 30),  # Joint 2 angles
        ...
        self.observation_space = 9  # [End Effector x, y, z, Target x, y, z,  Obstacle x, y, z]
    ...
    def step(self, action_steps):
        ...
        if distance_to_obstacle < 0.0001:
            reward = -10
